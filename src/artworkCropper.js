...
const artworkShowcase = {
	...
	downloadImages: async function () {
		...
		if (inputImage.file.type == 'image/gif') {
			...
			inputImage.file.type == 'image/png' ||
			inputImage.file.type == 'image/apng'
		) {
			const bigCanvas = await patchTransparency(
				artworkShowcase.bigCanvas.canvas,
				inputImage.file.type
			);
			zip.file(`1_${inputImage.file.name}`, bigCanvas, {
				base64: true,
			});

			const smallCanvas = await patchTransparency(
				artworkShowcase.smallCanvas.canvas,
				inputImage.file.type
			);
			zip.file(`2_${inputImage.file.name}`, smallCanvas, {
				base64: true,
			});
			...
			zip.file(
				...
				artworkShowcase.bigCanvas
					.toDataURL(inputImage.file.type, 1)
					.split(';base64,')[1],
				{
					base64: true,
				}
...
async function as_createGifs(zip, gifs, currentGif) {
	...
	let gifjs = new GIF({
		...
		transparent: document.getElementById('gifSloppyTransparent').checked
			? undefined
			: '#000000',
	});

	gifjs.on('finished', async function (blob) {
		const gifFile = await patchTransparency(blob);
		zip.file(`${currentGif}_${inputImage.file.name}`, gifFile, {
			base64: true,
		});
	...
	let smallImgHeight = rightPanel.toggleSmall.checked
	...
	for (let i = 0; i < gifs.length; i++) {
		if (document.getElementById('gifSloppyFix').checked) {
			background.clear();
		}
...
async function patchTransparency(data, imageType) {
	return new Promise((resolve) => {
		if (imageType == 'image/png' || imageType == 'image/apng') {
					...
					if (imageType == 'image/png' || imageType == 'image/apng') {
						for (let i = binaryImage.length - 1; i != 0; i--) {
							if (binaryImage.charAt(i) == '\x00') {
								binaryImage =
									binaryImage.substring(0, i) +
									'\x01\x49\x45\x4E\x44\x00\xD1\x1A\x4F\xE1';
						...
						binaryImage =
							binaryImage.substring(0, binaryImage.length - 2) +
							'\x21';
					}

					resolve(window.btoa(binaryImage));
				};
			...
			patcher.onload = function () {
				...
				for (let i = binaryImage.length - 1; i != 0; i--) {
					if (binaryImage.charAt(i) != '\x00') {
						binaryImage = binaryImage.substring(0, i) + '\x21';
						break;
					}
				}
			...
			patcher.readAsArrayBuffer(data);
		}
	});
}
