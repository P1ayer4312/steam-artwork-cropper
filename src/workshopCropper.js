...
function ws_cropImages(zip, canvasEl, sliceNum, isImageSmall) {
	...
	patcher.onload = function() {
		...
		binaryImage = binaryImage.substring(0, binaryImage.length - 1) + '\x21';
...
function ws_cropGifs(zip, gifs, currentSlice, isImageSmall) {
	...
	let gifjs = new GIF({
		...
		transparent: document.getElementById('gifSloppyTransparent2').checked
			? undefined
			: '#000000'
	});
	...
	for (let i = 0; i < gifs.length; i++) {
		if (document.getElementById('gifSloppyFix2').checked) {
			background.clear();
		}
